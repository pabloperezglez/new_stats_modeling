# ------------------------------------------------------------
# Chunk 1 – Load, reproject, and extract ONE month (temperature)
# ------------------------------------------------------------
library(terra)
library(sf)
...
# 2. Separate temperature layers (first 72 layers)
temp_layers <- era5[[1:72]]

# 3. Reproject the FIRST temperature layer (January 2015) to EPSG:3338
temp_jan <- project(temp_layers[[1]], "EPSG:3338", method = "bilinear")

# 4. Extract area‑weighted mean per hex for this single month
...
# Build a small lookup table with exact date strings
...
# Read the already-merged shipping + ice
...
# Save the complete analytic dataset
...
# =================================================================
# Fix time in data (it was in seconds since 1970, now it is in days)
# =================================================================

data_path <- "Downloads/Stats Modeling project/Data"
df <- readRDS(file.path(data_path, "analytic_dataset.rds"))

# Convert seconds since 1970 -> proper Date
df$date <- as.Date(as.POSIXct(df$date, origin = "1970-01-01", tz = "UTC"))

# Verify
cat("First 6 dates after conversion:\n")
print(head(df$date))
cat("Unique dates:", length(unique(df$date)), "\n")
